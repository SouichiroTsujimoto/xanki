...
| スマート学習の評価ボタンが 2 行に折り返し、操作エリアにグレー背景 | 4 列 grid + `min-width: 160px` + stage 下端のグラデーション背景 | flex 1 行・コンパクト幅・背景透過 |
| 評価ボタンの常時塗りつぶしが重い | 段階別の常時背景色 | 5 つとも透過 + グレー破線枠、ホバー時のみアクセント背景 |
| 評価ボタンにマウス近接の拡大アニメが欲しい | 静的 flex ボタン列 | React Bits `Dock`（`motion`）を `LearnMode` に統合 |
| スマート学習の評価 Dock がマウス近接で拡大・パネル高さが変わる | `Dock`（`motion`）の `useSpring` / `useTransform` による近接拡大 | 静的 `.leitner-grade-toolbar` + `.leitner-grade-btn` に戻し、見た目（破線枠・ホバーアクセント）は維持 |
| スマート学習の評価ボタン高さが不揃い（AI ボタンが低い・ラベル折り返しで段差） | `align-items: flex-end` + `min-height` のみで内容高さ任せ | `align-items: stretch` + 固定 `height: 84px`、ラベル `white-space: nowrap` |
| スマート学習の評価ボタン横幅が極端に狭い | `flex: 1 1 92px` + `max-width: 112px` + `review.css` の `min-width: 0` で shrink | 5 列 grid（`minmax(6.5rem, 1fr)`）+ 狭幅は AI 全幅 + 採点 2 列 grid |
