...
- **iOS (C6)**: **ASWebAuthenticationSession**（`AuthSessionPlugin`）で `/auth/desktop-sign-in`（return なし）→ OAuth → `/auth/desktop-callback` → **HTTP 302** で **`xanki://auth/callback?token=...`** → Preferences 保存 → `Authorization: Bearer`。OAuth 開始前に `/auth/desktop-sign-in` は `APP_URL` origin へ canonical redirect し、better-auth state cookie と Google callback の host を一致させる。`@capacitor/browser`（SFSafariViewController）は OAuth 深リンクに不向き
- iOS API 通信: シミュレータ dev の API 既定は `http://localhost:8787`。通常の Cloud REST / blob 取得 / ログアウトは `NativeHttpPlugin`（URLSession）で実行し、WKWebView の `http://localhost` アプリ資産ホスト、CORS preflight、Cloudflare/Vite dev 前段の `OPTIONS` 応答に依存しない。`/api/*` CORS は WebView / Desktop / fallback 用に `http://localhost` と `capacitor://localhost` を明示許可し、private network preflight には `Access-Control-Allow-Private-Network: true` を返す
...
PUT    /api/decks/:id/cards/order    { cardIds: string[] }  → デッキ内カード並べ替え
...
- `GET /api/cards` は `sort_order ASC, created_at ASC` で返す（ユーザー定義の並び順）
- `PUT /api/decks/:id/cards/order` の `cardIds` は当該デッキの未削除カード ID 集合と**完全一致**必須（欠落・余分・重複は 400 `card_order_mismatch`）。成功時 `sort_order` を 0..n-1 に書き戻し、`finishMutation` で SSE revision を進める
- 新規カードの `sort_order` はデッキ内 `MAX(sort_order)+1`（末尾）
...
CREATE TABLE cards ( user_id, id, deck_id, kind, content, answer, image_hash, ocr_text, ocr_data,
                     masks, note, source_hint, starred, sort_order, created_at, updated_at, deleted_at,
...
```
POST /api/ai/qa-generate      { text, count?, kind: 'qa' | 'choice' }              → { items: [...] }  # 後方互換
POST /api/ai/cards-generate   { text?, images?: [{ blobHash }], count?, kind: 'qa', tier: 'fast' | 'thinking' } → { items: [...] }
...
| tier | UI 表示 | モデル env | クレジット消費 |
|------|---------|-----------|---------------|
| `fast` | **高速AI** | `AI_MODEL_FAST` → `AI_MODEL` → 既定 flash | 1 |
| `thinking` | **熟考AI** | `AI_MODEL_THINKING` → `AI_MODEL` → 既定 flash | 2 |

- `/api/ai/ask` と `/api/ai/qa-generate` は常に **fast** tier
- `/api/ai/cards-generate` はリクエストの `tier` に従う（既定 UI: thinking）

### `POST /api/ai/cards-generate`

- `text` または `images` のいずれか必須（両方可）
- `text` 最大 80,000 文字
...
| `AI_MODEL` | 後方互換の既定モデル。`google-ai-studio/gemini-2.5-flash` |
| `AI_MODEL_FAST` | fast tier。未設定時は `AI_MODEL` |
| `AI_MODEL_THINKING` | thinking tier。未設定時は `AI_MODEL` |
