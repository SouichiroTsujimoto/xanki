...
- **iOS (C6)**: **ASWebAuthenticationSession**（`AuthSessionPlugin`）で `/auth/desktop-sign-in`（return なし）→ OAuth → `/auth/desktop-callback` → **HTTP 302** で **`xanki://auth/callback?token=...`** → Preferences 保存 → `Authorization: Bearer`。`@capacitor/browser`（SFSafariViewController）は OAuth 深リンクに不向き
