...
|--------|------|
| 意味・原則・トークン名 | 本 spec |
| **値（色・半径・影・border/focus 幅）** | [`packages/ui/src/styles/tokens.css`](../../packages/ui/src/styles/tokens.css) |
| focus 実装 | [`packages/ui/src/styles/focus.css`](../../packages/ui/src/styles/focus.css) |
| コンポーネントスタイル | [`packages/ui/src/styles/globals.css`](../../packages/ui/src/styles/globals.css)（`base.css` + `focus.css` + `components/*.css`） |
| 共有 UI コンポーネント | [`packages/ui`](../../packages/ui/)（`@xanki/ui`） |
| コンポーネント選択 | [`ui-components.md`](./ui-components.md) |
...
- `tokens.css` — デザイントークン（spacing、border/focus、study flip、スマート学習アクセント等）
- `base.css` — フォームリセット、`.sr-only`、`.eyebrow`、`:user-invalid` / `:autofill`
- `focus.css` — キーボード focus（`:focus-visible`）の集中定義。**glow 禁止**
...
| ボーダー幅（通常） | `--border-width-default` | — |
| ボーダー幅（強調・選択） | `--border-width-strong` | — |
| フォーカス縁 | `--color-ring` + `--focus-ring-width` / `--focus-ring-offset` | — |
| 選択・active 縁 | `--color-accent-outline` | — |
...
| 状態 | 用途 | 実装 |
|------|------|------|
| Default border | カード・パネル・ghost ボタン | `var(--border-width-default)` + `--color-border` または `--color-border-strong` |
| Strong border | 強調枠・brand mark | `var(--border-width-strong)` + `--color-border-strong` または `--color-black` |
| Selected / active | 選択中・実行中 | `var(--border-width-strong)` + `--color-accent-outline` |
| Focus (keyboard) | 全 interactive | [`focus.css`](../../packages/ui/src/styles/focus.css) の `:focus-visible` パターン。**box-shadow glow 禁止** |

色の意味:

- `--color-ring` — キーボードフォーカス縁（outline）
- `--color-accent-outline` — 選択・active 状態の border（値は `--color-ring` と同じ `#1d1d1f` だが用途を分離）

新規 CSS では `outline: Npx` / `border: 1.5px` 等の直書きを禁止し、上記トークンを使う。検証: `pnpm check:design`。

## AI 向けデザイン skill 利用

| 状況 | 使う skill |
|------|-----------|
| 既存 UI パターンに該当（ボタン、ダイアログ、カード一覧等） | [ui-components.md](./ui-components.md) を参照。**frontend-design は使わない** |
| 新画面種別（オンボーディング刷新等） | frontend-design でレイアウト方向性。決定した spacing / border / focus は tokens + 本 spec に逆流 |
| focus / a11y / motion | modern-web-guidance を先に search（`focus-visible` 等） |

不変の aesthetic: **白ベース + Chartreuse アクセント + システムフォント**。frontend-design で palette やフォントファミリを変更しない。
...
| **スマート学習** | 横断復習予定、ヒーローカード、デッキ別一覧 |
| **設定** | 権限・環境設定 |

- **デッキ学習ハブ**・**スマート学習ハブ**・**設定タブ**の縦スクロールバーは非表示（スクロール自体は可能）
...
| デッキ学習（セッション） | 手段名、「戻る」 |
| スマート学習（ハブ） | スマート学習、今日 N 件 |
| スマート学習（セッション） | 復習中、「戻る」 |
| 設定 | 設定 |

Tray **今日の復習: N件** → **スマート学習**タブ（`navigate: "leitner"`）。rail バッジもスマート学習のみ。
...
- [ ] フォーカス表示に box-shadow glow を使っていない（outline + focus トークンのみ）
- [ ] border 幅は `--border-width-default` / `--border-width-strong` のみ（直書き px 禁止、`pnpm check:design` 通過）
